...
const { syncMeetingToBrain } = require("./lib/brain-sync");
...
app.whenReady().then(async () => {
  ...
  console.log("Auth initialized, has session:", hasSession);
...
async function createDesktopSdkUpload() {
  try {
    ...
    console.error(
      ...
      JSON.stringify(error.errors || error.message || error),
    );
    if (error.response) {
...
async function updateNoteWithRecordingInfo(recordingId) {
  try {
    ...
    if (meeting.transcript && meeting.transcript.length > 0) {
      ...
      // Sync to Brain knowledge system (async, don't block)
      syncMeetingToBrain(meeting).catch((err) => {
