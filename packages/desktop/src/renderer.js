...
function registerIpcCleanup(unsubscribe) {
  if (typeof unsubscribe === "function") {
    ipcUnsubscribers.push(unsubscribe);
  }
}
...
async function initAuthUI() {
  ...
  // Make the avatar clickable with cursor pointer
  userAvatar.style.cursor = "pointer";

  // Check initial auth state
  try {
    const isAuthenticated = await window.electronAPI.auth.isAuthenticated();
    console.log("[Auth UI] Initial auth state:", isAuthenticated);
    updateAvatarUI(isAuthenticated);
  } catch (error) {
    console.error("[Auth UI] Error checking initial auth state:", error);
  }

  // Listen for auth state changes
  registerIpcCleanup(
    window.electronAPI.auth.onAuthStateChanged((data) => {
      console.log("[Auth UI] Auth state changed:", data);
      currentAuthUser = data.user || null;
      updateAvatarUI(data.isAuthenticated, data.user);
    }),
  );
...
async function handleAvatarClick(e) {
  ...
  try {
    ...
    if (isAuthenticated) {
      // Show dropdown menu with logout option
      if (isAuthMenuOpen) {
        closeAuthMenu();
      } else {
        showAuthMenu();
      ...
      console.log("[Auth UI] Starting login flow...");
      showLoginLoading();

      try {
        const result = await window.electronAPI.auth.login();
        console.log("[Auth UI] Login successful:", result);
        updateAvatarUI(true);
        showToast("Signed in successfully!");
      } catch (error) {
        console.error("[Auth UI] Login failed:", error);
        updateAvatarUI(false);
        showToast("Sign in failed. Please try again.");
      }
    }
  } catch (error) {
    console.error("[Auth UI] Error handling avatar click:", error);
  }
}

// Update avatar UI based on auth state
function updateAvatarUI(isAuthenticated, user = null) {
  const userAvatar = document.querySelector(".user-avatar");
  if (!userAvatar) return;
  ...
  if (isAuthenticated && user && user.imageUrl) {
    ...
    userAvatar.innerHTML = `
      <img src="${user.imageUrl}" alt="${user.name || "User"}" class="avatar-img" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover;">
    `;
    userAvatar.title = user.name || user.email || "Signed in";
  } else if (isAuthenticated) {
    // Authenticated but no user image - show a colored avatar
    userAvatar.innerHTML = `
      <svg width="32" height="32" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect width="40" height="40" rx="20" fill="#6947BD"/>
        ...
        <path d="M12 31C12 26.0294 15.5817 22 20 22C24.4183 22 28 26.0294 28 31" stroke="white" stroke-width="4"/>
      </svg>
    `;
    userAvatar.title = "Signed in - Click to sign out";
  } else {
    // Not authenticated - show gray placeholder
    userAvatar.innerHTML = `
      <svg width="32" height="32" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect width="40" height="40" rx="20" fill="#f0f0f0"/>
        <path d="M20 20C22.7614 20 25 17.7614 25 15C25 12.2386 22.7614 10 20 10C17.2386 10 15 12.2386 15 15C15 17.7614 17.2386 20 20 20Z" fill="#a0a0a0"/>
    ...
    `;
    userAvatar.title = "Click to sign in";
  }
}

// Show loading state on avatar during login
function showLoginLoading() {
  const userAvatar = document.querySelector(".user-avatar");
  if (!userAvatar) return;

  userAvatar.innerHTML = `
    <svg width="32" height="32" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" class="avatar-loading">
      <rect width="40" height="40" rx="20" fill="#f0f0f0"/>
      <circle cx="20" cy="20" r="8" stroke="#6947BD" stroke-width="2" stroke-linecap="round" stroke-dasharray="25 25">
        <animateTransform attributeName="transform" type="rotate" from="0 20 20" to="360 20 20" dur="1s" repeatCount="indefinite"/>
      </circle>
    </svg>
  `;
  userAvatar.title = "Signing in...";
}

// Show auth menu dropdown
function showAuthMenu() {
  ...
  menu.innerHTML = `
    <div class="auth-menu-item" id="auth-logout">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  menu.style.top = `${avatarRect.bottom + 8}px`;
  ...
  document.getElementById("auth-logout").addEventListener("click", async () => {
    console.log("[Auth UI] Logging out...");
    closeAuthMenu();

    try {
      await window.electronAPI.auth.logout();
      console.log("[Auth UI] Logged out successfully");
      updateAvatarUI(false);
